...
class TestDefaultDictTTL(unittest.TestCase):
    ...
    def test_is_expired(self, patched_time_sleep):
        # dict_ttl = DefaultDictTTL(10, a=1, b=2)

        now = time.time()
        # self.dict_ttl = DefaultDictTTL(10, a=1, b=2)
    ...
    def test_dict_union(self):
        self.data = {'c':3}
        self.assertEqual(DefaultDictTTL(time_to_live=10).dict_union(self.dict_ttl,self.data),{'a':1,'b':2,'c':3})
        self.assertNotEqual(DefaultDictTTL(time_to_live=10).dict_union(self.dict_ttl, self.data), {'a': 1, 'b': 2})

    def test_dict_intersection(self):
        self.data = {'b':2,'c': 3}
        self.assertEqual(DefaultDictTTL(time_to_live=10).dict_intersection(self.dict_ttl, self.data), {'b': 2})
        self.assertNotEqual(DefaultDictTTL(time_to_live=10).dict_intersection(self.dict_ttl, self.data), {'a':1,'b': 2})
