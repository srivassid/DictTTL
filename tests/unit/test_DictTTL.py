...
class TestDictTTL(unittest.TestCase):
    def setUp(self):
        self.dict_ttl = DictTTL(10, a=1, b=2)

    @patch('time.sleep', return_value=None)
    def test_is_expired(self, patched_time_sleep):
        # dict_ttl = DictTTL(10, a=1, b=2)
    ...
    def test__set_item(self):
        ...
        self.dict_new_ttl = DictTTL(10,self.data)
    ...
    def test_sort_by_value(self):
        ...
        self.dict_ttl = DictTTL(10, self.data).sort_by_value(reverse=False)
    ...
    def test_dict_union(self):
        self.data = {'c':3}
        self.assertEqual(DictTTL(time_to_live=10).dict_union(self.dict_ttl,self.data),{'a':1,'b':2,'c':3})
        self.assertNotEqual(DictTTL(time_to_live=10).dict_union(self.dict_ttl, self.data), {'a': 1, 'b': 2})

    def test_dict_intersection(self):
        self.data = {'b':2,'c': 3}
        self.assertEqual(DictTTL(time_to_live=10).dict_intersection(self.dict_ttl, self.data), {'b': 2})
        self.assertNotEqual(DictTTL(time_to_live=10).dict_intersection(self.dict_ttl, self.data), {'a':1,'b': 2})
